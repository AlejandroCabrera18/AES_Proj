...
void add_round_key(byte *byte_array, byte *round_array) {
	// This function is used to add the round key to the current state
	// byte round_array = get_round_key();
	for (size_t i = 0; i < 16; i++) {
		byte_array[i] = byte_array[i] ^ round_array[i]; // ^ represents XOR
	}
}

// S-box ( Substitute a single byte )
byte substitute_byte(byte byte_to_substitute) {
	// This is the function that substitutes the old hex number with a new one using the S-box
	byte S_Box[16][16] = {
...
void substitute_bytes(byte *byte_array) {
	...
	for (size_t i = 0; i < 4; i++) {
		...
		byte_array[i + 4] = substitute_byte(byte_array[i + 4]);
		byte_array[i + 8] = substitute_byte(byte_array[i + 8]);
		byte_array[i + 12] = substitute_byte(byte_array[i + 12]);
		//cout << "0x" << hex << byte_array[i] << " " << "0x" << hex << byte_array[i + 4] << " " << "0x" << hex
			//<< byte_array[i + 8] << " " << "0x" << hex << byte_array[i + 12] << endl;
	}
}